...
| Taux d'erreur 5xx | > 5% des requêtes sur une fenêtre glissante de 15 min pour un tenant (minimum 20 requêtes observées, pour ne pas alerter sur 1 échec isolé) | Email à `ALERT_EMAIL` | ✅ livré (2026-09) — `_track_tenant_5xx()`/`_check_tenant_5xx_rate()` dans `app/middlewares/metrics.py` détectent le dépassement en mémoire par tenant (déclencheur local, sans I/O sur le chemin de la requête), puis relisent un agrégat cross-réplica via des compteurs Redis (`_bump_tenant_redis_counters`/`_read_tenant_redis_rate`, un bucket par minute, écrit en fire-and-forget à chaque requête) avant d'envoyer l'alerte — le taux rapporté reflète la flotte entière, pas juste le processus qui a détecté le dépassement. `enqueue_job("send_tenant_5xx_alert", ...)` (dédupliqué par tenant+heure via `_job_id`) déclenche l'envoi côté worker Arq (`send_tenant_5xx_alert` dans `app/workers/tasks.py`) — jamais dans le event loop de la requête. Testé (`tests/test_tenant_5xx_alert.py`). |
...
- Le déclenchement de l'alerte 5xx reste local par réplica (chaque instance décide seule si SON trafic dépasse le seuil) — mais le taux rapporté dans l'email, lui, est désormais agrégé sur toute la flotte via Redis (2026-09). Limite résiduelle : un tenant dont les erreurs sont réparties également entre plusieurs réplicas, sans qu'aucune ne dépasse seule le seuil localement, ne déclenchera toujours pas l'alerte — lire l'agrégat sur CHAQUE requête pour fermer ce dernier écart ajouterait un aller-retour Redis à chaque requête de toute l'application, jugé disproportionné pour ce cas marginal. À revoir si ce scénario s'avère réel en production.
