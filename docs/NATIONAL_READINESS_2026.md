...
|---|---:|---:|---:|---|
| SaaS multi-établissement (usage actuel) | 64/100 | 72/100 | **78/100** | MFA réellement appliqué au login (pas seulement côté client), PDF serveur pour 2 documents officiels, imports CSV asynchrones sur les 3 entités principales, pagination SQL brut confirmée partout où le volume le justifie. |
| Prêt pour échelle nationale | 27/100 | 35/100 | **45/100** | Hiérarchie institutionnelle MFA-protégée complète (Ministère/Inspecteur/Région/Préfecture/Commune), pattern d'import asynchrone étendu et documenté. Module université mergé (base seulement). PWA reste la seule décision non technique en attente. |
...
- **Module université** (P2-2) — la base RLS/structure est mergée, la couverture fonctionnelle (facultés, UE, crédits, semestres) ne l'a pas encore été auditée.
...
Les 3 pistes de la version précédente de ce document (étendre l'import async aux parents/enseignants, PDF serveur pour un 2e document officiel, décision PWA) sont toutes traitées : les deux premières sont résolues (#211, #212), la troisième reste un arbitrage produit en attente, pas une tâche technique. Aucune nouvelle priorité n'a encore été validée pour le prochain cycle — candidats possibles listés dans "Ce qui reste ouvert" ci-dessus (bulletins en masse vers Arq, reste des générateurs jsPDF, audit fonctionnel du module université).
