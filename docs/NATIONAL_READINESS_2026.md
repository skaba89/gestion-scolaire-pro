...
| P1-3 | PWA/offline désactivé en production | 🟡 **Partiellement résolu** (voir §7) | Le constat original (SW "killer" volontaire, aucun manifest, app non installable) ne décrit plus l'état actuel du code sur `main` : `public/sw-schoolflow.js` est enregistré en production (`src/main.tsx`), stratégie Cache-First/Network-First, exclut explicitement `/api/v1/`, `.onrender.com` etc. — le risque technique historique ("Failed to fetch" en cold start Render) qui avait motivé la désactivation est traité. `public/manifest.webmanifest` existe et est lié dans `index.html` — l'app est installable. Reste non fait, et c'est bien un choix produit cette fois : lecture de données métier hors-ligne et file de synchronisation différée (voir `docs/mobile/PWA_AUDIT.md` §2-3, marqués 🟠/🔴 dans ce document-là). |
...
| Prêt pour échelle nationale | 27/100 | 35/100 | **62/100** | Hiérarchie institutionnelle MFA-protégée complète, imports asynchrones étendus, **module université désormais fonctionnellement complet** (facultés, semestres, prérequis, crédits ECTS, porte de progression), **et PWA/SW fiabilisé + installable déjà en prod** (voir §7) — seule la lecture hors-ligne des données métier (choix produit, périmètre à définir) reste ouverte, dernière ligne non verte de ce document. |

---

## 3. Ce qui reste ouvert

- **P1-3 (reste)** — le risque technique (SW fiable, installabilité) est réglé, voir §7 ; reste un vrai choix produit non tranché : construire la lecture hors-ligne des données métier et la file de sync différée (`docs/mobile/PWA_AUDIT.md` §3, points 1-2), ou rester en lecture-seule assets statiques.
...
**Ce qui reste hors périmètre** : la part produit de P1-3 (lecture hors-ligne des données métier, voir §7) — un choix explicitement mis de côté, pas une dette technique. "Tout vert" sur ce document signifie donc : tous les constats techniques résolus, cette seule décision produit exceptée et assumée, pas oubliée.
...
## 7. P1-3 (PWA/offline) — le constat était périmé, pas le produit

En rouvrant ce point pour trancher la décision produit qu'il réclamait, lecture directe du code sur `main` (pas seulement de `docs/mobile/PWA_AUDIT.md`, dont les dernières entrées datées remontent au 22/08) :

- **`src/main.tsx`** enregistre `public/sw-schoolflow.js` en production (`!import.meta.env.DEV`), avec un délai de 3 s sur les domaines `*.onrender.com` pour laisser d'abord tourner un « SW killer » (`public/sw.js`) qui nettoie tout Service Worker/cache existant — cette séquence *tue* l'ancien SW cassé puis *réactive* un SW fiabilisé juste après, elle ne désactive rien de façon permanente.
- **`public/sw-schoolflow.js`** implémente exactement le correctif que P1-3 réclamait à l'origine : Cache-First pour les assets statiques, Network-First pour la navigation, et surtout `API_PATTERNS` exclut explicitement `/api/v1/`, `.onrender.com`, `.railway.app`, `.fly.dev` — jamais d'interception des appels API. Le commentaire en tête du fichier documente lui-même la cause de l'incident d'origine (`event.respondWith(fetch(...))` de l'ancienne implémentation Workbox interceptait tout, causant des "Failed to fetch" aux cold starts Render) et pourquoi cette version l'évite.
- **`public/manifest.webmanifest`** existe, lié dans `index.html` — l'app est installable (page `/install` fonctionnelle).

**Conclusion** : le risque technique qui avait motivé la décision "on désactive tout" (un SW mal conçu cassait l'API en prod) est déjà traité, avec du code déployé, pas seulement de la doc. La doc de ce document (P1-3 précédemment marqué ❌, "hors périmètre") n'avait simplement pas été mise à jour pour refléter ce travail. Corrigé ci-dessus (🟡).

**Ce qui reste un vrai choix produit, non tranché ici** : `docs/mobile/PWA_AUDIT.md` §2-3 documente que ni la lecture de données métier hors-ligne (classes du jour, listes d'élèves — actuellement juste du cache mémoire React Query, perdu à la fermeture de l'onglet) ni une file d'actions offline (brouillons de présence rejoués au retour réseau) ne sont construites. La brique applicative existe déjà et est testée (`src/offline/__tests__/outbox.test.ts`, 16/16) mais n'est branchée nulle part dans l'UI. C'est un chantier produit à part entière (persistance IndexedDB, UX de synchronisation, arbitrage sur quelles données mettre en cache) — proposé mais non lancé dans cette passe, faute de décision explicite sur le périmètre voulu (lecture seule uniquement ? présence uniquement ? autre chose ?).
