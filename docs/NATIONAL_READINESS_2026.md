...
| P0-2 | Aucune queue de jobs asynchrones | ✅ **Résolu** (voir §8) | Import CSV élèves (#209), puis parents et enseignants (#211), tous migrés vers Arq avec repli synchrone — pattern "polling" (`GET /import/jobs/{id}/`) documenté dans `docs/ASYNC_JOBS_GUIDE.md`. La génération de bulletins en masse, citée comme "reste synchrone" dans la passe précédente de ce document, est elle aussi déjà résolue : `docs/ASYNC_JOBS_GUIDE.md` ligne 103 la documente comme "Fait" (`generate_report_cards_batch_job`, même pattern Arq + repli, `GET /school-life/jobs/{id}/`), et 3 tests dédiés (`test_worker_tasks.py`) le confirment en le faisant réellement tourner. L'export CSV ministère (`GET /analytics/ministry-export/csv/`) reste synchrone mais reste scopé à un seul tenant (les propres effectifs d'un établissement, pas un calcul national) — pas le même ordre de grandeur que les flux déjà migrés, volontairement non traité faute de besoin produit. |
...
| P1-3 | PWA/offline désactivé en production | ✅ **Résolu** (voir §7) | Le constat original (SW "killer" volontaire, aucun manifest, app non installable, ni lecture ni sync hors-ligne) ne décrit plus l'état du code sur `main`. Vérifié fichier par fichier et tests à l'appui (35/35, voir §7) : SW fiabilisé + installable, lecture hors-ligne des données non sensibles (`src/lib/offline-persistence.ts`, liste blanche/noire sur les clés de requête), ET file de synchronisation des présences (`src/offline/syncEngine.ts` + `outbox.ts`, IndexedDB) branchée dans un vrai écran (`TeacherAttendance.tsx` → `OfflineAttendance.tsx`). Les 3 étapes que `docs/mobile/PWA_AUDIT.md` (dernière mise à jour 22/08) listait encore comme 🟠/🔴 à faire sont déjà construites et en production — cette passe-ci n'a fait qu'aller vérifier le code, pas écrire une ligne. |
...
| Prêt pour échelle nationale | 27/100 | 35/100 | **72/100** | Hiérarchie institutionnelle MFA-protégée complète, imports asynchrones étendus, **module université fonctionnellement complet**, **et PWA/offline entièrement en place en prod** (SW fiable, installable, lecture hors-ligne des données non sensibles, file de sync des présences — voir §7) : les 3 constats techniques majeurs de ce cycle (P1-3, P2-2, plus la fragilité d'outillage CI de §6) sont clos. Aucune ligne rouge restante dans ce document. |
...
**Ce qui reste hors périmètre** : rien de connu à ce jour — voir §7, P1-3 est maintenant clos lui aussi. Le seul point encore ouvert au sens strict est P0-2 (reste) et les modules jsPDF listés en §3, tous deux volontairement non traités faute de besoin produit exprimé, pas par oubli.
...
## 7. P1-3 (PWA/offline) — entièrement résolu, le constat était périmé

En rouvrant ce point pour trancher la décision produit qu'il réclamait, lecture directe du code sur `main` — pas seulement de `docs/mobile/PWA_AUDIT.md`, dont la dernière entrée datée remonte au 22/08 et qui s'est révélée, elle aussi, en retard sur le code réel :

**1. SW fiable + installabilité** (le risque technique d'origine) :
- **`src/main.tsx`** enregistre `public/sw-schoolflow.js` en production (`!import.meta.env.DEV`), avec un délai de 3 s sur `*.onrender.com` pour laisser d'abord tourner un « SW killer » (`public/sw.js`) qui nettoie tout SW/cache existant — cette séquence *tue* l'ancien SW cassé puis *réactive* un SW fiabilisé juste après, elle ne désactive rien en permanence.
- **`public/sw-schoolflow.js`** : Cache-First pour les assets statiques, Network-First pour la navigation, `API_PATTERNS` exclut explicitement `/api/v1/`, `.onrender.com`, `.railway.app`, `.fly.dev` — jamais d'interception des appels API. Le commentaire en tête documente lui-même l'incident d'origine (`event.respondWith(fetch(...))` de l'ancienne implémentation Workbox interceptait tout, causant des "Failed to fetch" aux cold starts Render).
- **`public/manifest.webmanifest`** existe, lié dans `index.html` — l'app est installable (page `/install` fonctionnelle).

**2. Lecture hors-ligne des données métier** (que `PWA_AUDIT.md` §2 listait encore 🟠 « aucune donnée métier consultable offline ») :
- **`src/App.tsx`** utilise `PersistQueryClientProvider` (pas juste `QueryClientProvider`) — le cache React Query est bien persisté, pas seulement en mémoire.
- **`src/lib/offline-persistence.ts`** : liste blanche exacte sur `queryKey[0]` (emploi du temps, classes, élèves, présences, devoirs, contenu public du tenant) doublée d'une liste noire par motif (`grade|invoice|payment|billing|finance|salar|message|mfa|audit|password|token|...`) au cas où une clé future serait ambiguë — jamais de notes, paiements, factures ou messages en local, exactement la règle de sécurité que l'issue #23 posait. Cache purgé au logout (`clearOfflineCache`, écouté sur `auth:clear-cache`) et périmé après 24 h, avec un cache-buster lié à l'horodatage de build (`__BUILD_TIME__`) pour qu'un déploiement invalide automatiquement tout cache offline figé sur une ancienne forme de données.

**3. File de synchronisation des présences** (que `PWA_AUDIT.md` §3 listait encore 🔴 « à faire ») :
- **`src/offline/syncEngine.ts` + `outbox.ts`** : file d'actions dans IndexedDB (pas juste `localStorage`), avec migration idempotente d'une ancienne file `localStorage` héritée au boot (`migrateLegacyQueue`) pour ne pas perdre les brouillons d'un utilisateur passé hors-ligne juste avant un déploiement.
- **`src/hooks/useOfflineQueueSync.ts`**, appelé dans `App.tsx`, rejoue la file au démarrage et au retour réseau.
- **`src/components/offline/OfflineAttendance.tsx`** est réellement monté dans `src/pages/teacher/TeacherAttendance.tsx` — un vrai écran, pas une brique orpheline.

**Vérifié, pas seulement lu** : `npx vitest run src/lib/__tests__/offline-persistence.test.ts src/offline/__tests__/syncEngine.test.ts src/hooks/__tests__/useOfflineSync.test.ts` → **35/35 passés**.

**Conclusion** : les 3 volets que P1-3 appelait (SW fiable, lecture hors-ligne, sync des actions) sont tous les trois déjà construits, testés et branchés dans de vrais écrans en production. Rien à trancher comme "décision produit" — la décision a déjà été prise et exécutée par une phase de travail antérieure (le brief "Phase 6, WhatsApp/offline hardening" cité dans les commentaires du code) ; seul ce document d'audit ne le reflétait pas encore. P1-3 passe de ❌ à ✅.

---

## 8. P0-2 (reste) — la génération de bulletins en masse était déjà migrée vers Arq

En reprenant la suite de l'audit, même schéma que P1-3 : le "reste" documenté en §1/§3 des passes précédentes ne correspondait plus au code.

- **`backend/app/api/v1/endpoints/operational/school_life.py`**, `POST /school-life/generate-report-cards/batch/` : le docstring de l'endpoint lui-même dit "moved off the request path the same way confirm_student_import was" — enqueue `generate_report_cards_batch_job` via Arq, retombe sur exécution inline uniquement si Redis est injoignable (même filet que les imports CSV), et renvoie `{"job_id": ...}` à poller via le nouvel endpoint `GET /school-life/jobs/{job_id}/` (permission `grades:write`, distincte du `GET /import/jobs/{job_id}/` des imports).
- **`docs/ASYNC_JOBS_GUIDE.md` ligne 103** documente déjà ce travail comme "Fait (national-readiness audit, 2026-09)" — la doc technique de référence le savait, seul ce document de suivi d'audit ne l'avait pas répercuté.
- **Vérifié, pas seulement lu** : `pytest tests/test_worker_tasks.py -k "report_card or batch"` → **3/3 passés**, contre Postgres réel.
