...
@router.get("/fix-schema/", include_in_schema=False)
def fix_schema(db: Session = Depends(get_db)):
    """Emergency endpoint: inspect users table columns and add missing ones."""
    from sqlalchemy import text
    result = {"action": "fix-schema"}

    # 1. List current columns in users table
    try:
        cols = db.execute(text(
            "SELECT column_name, data_type FROM information_schema.columns "
            "WHERE table_name = 'users' ORDER BY ordinal_position"
        )).fetchall()
        result["current_columns"] = [{"name": c[0], "type": c[1]} for c in cols]
    except Exception as e:
        result["current_columns_error"] = str(e)

    # 2. Try adding tenant_id directly (simplest possible ALTER TABLE)
    fixes = []
    missing_cols = [
        ("tenant_id", "UUID"),
    ...
    for col_name, col_def in missing_cols:
        try:
            ...
            fixes.append({"column": col_name, "status": "ok"})
        except Exception as e:
            db.rollback()
            fixes.append({"column": col_name, "status": "error", "error": str(e)})

    try:
        db.commit()
        result["commit"] = "ok"
    except Exception as e:
        result["commit"] = f"error: {e}"
        db.rollback()

    result["fixes"] = fixes

    # 3. Re-check columns after fix
    try:
        cols = db.execute(text(
            "SELECT column_name, data_type FROM information_schema.columns "
            "WHERE table_name = 'users' ORDER BY ordinal_position"
        )).fetchall()
        result["columns_after_fix"] = [{"name": c[0], "type": c[1]} for c in cols]
    except Exception as e:
        result["columns_after_fix_error"] = str(e)
