...
def get_my_department(
    ...
    dept = _get_user_department(db, current_user["id"], current_user["tenant_id"])
    if not dept:
...
def department_dashboard(
    ...
    if class_ids:
        class_ids_param = ",".join([f"'{cid}'" for cid in class_ids])

        # Students
        stats["totalStudents"] = db.execute(text(f"""
            SELECT COUNT(DISTINCT e.student_id) FROM enrollments e
            WHERE e.class_id IN ({class_ids_param}) AND e.status = 'active'
        """)).scalar() or 0

        # Teachers
        stats["totalTeachers"] = db.execute(text(f"""
            SELECT COUNT(DISTINCT ta.teacher_id) FROM teacher_assignments ta
            WHERE ta.class_id IN ({class_ids_param}) AND ta.tenant_id = :tenant_id
        ...
        att = db.execute(text(f"""
            ...
            WHERE class_id IN ({class_ids_param}) AND date >= :since
...
def department_classrooms(
    ...
    tenant_id = current_user["tenant_id"]
    dept = _get_user_department(db, current_user["id"], tenant_id)
    if not dept:
        raise HTTPException(status_code=404, detail="Aucun département assigné")
...
def create_exam(
    ...
    """), {
        ...
        "term_id": body.term_id, "created_by": current_user["id"],
    }).scalar()
    db.commit()
