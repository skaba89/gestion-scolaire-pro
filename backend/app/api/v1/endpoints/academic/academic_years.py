...
def get_academic_year(
    ...
    ay = crud_academic.get_academic_year(db, ay_id, tenant_id=current_user["tenant_id"])
    if not ay:
...
def create_academic_year(
    ...
    return crud_academic.create_academic_year(db, ay_in, tenant_id=current_user["tenant_id"])
...
def update_academic_year(
    ...
    ay = crud_academic.update_academic_year(db, ay_id, ay_in, tenant_id=current_user["tenant_id"])
    if not ay:
        raise HTTPException(status_code=404, detail="Academic year not found")
    return ay
...
def delete_academic_year(
    ...
    success = crud_academic.delete_academic_year(db, ay_id, tenant_id=current_user["tenant_id"])
    if not success:
        raise HTTPException(status_code=404, detail="Academic year not found")
    return None
