...
def get_student(
    ...
    Permissions: students:read
    """
    student = crud_student.get_student(db, student_id, current_user["tenant_id"])
    if not student:
        raise HTTPException(
...
def create_student(
    ...
    existing = crud_student.get_student_by_registration(
        db, student.registration_number, current_user["tenant_id"]
    )
    if existing:
        raise HTTPException(
    ...
    return crud_student.create_student(db, student, current_user["tenant_id"])
...
def update_student(
    ...
    updated_student = crud_student.update_student(
        db, student_id, student_update, current_user["tenant_id"]
    )
...
def delete_student(
    ...
    Permissions: students:write
    """
    success = crud_student.delete_student(db, student_id, current_user["tenant_id"])
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Student not found"
