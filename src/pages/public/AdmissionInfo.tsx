...
const AdmissionInfo = () => {
  ...
  if (!tenant) {
    return (
      <div className="min-h-screen bg-muted/30 flex items-center justify-center">
        <Card className="max-w-md"><CardContent className="p-8 text-center"><GraduationCap className="w-12 h-12 text-muted-foreground mx-auto mb-4" /><h1 className="text-xl font-bold mb-2">Établissement non trouvé</h1><p className="text-muted-foreground mb-4">L'établissement que vous recherchez n'existe pas ou n'est plus actif.</p><Link to="/"><Button variant="outline">Retour à l'accueil</Button></Link></CardContent></Card>
      </div>
    );
  }

  const tenantTypeLabels: Record<string, string> = {
    primary_school: "École Primaire", middle_school: "Collège", high_school: "Lycée", university: "Université", training_center: "Centre de Formation", school: "Établissement Scolaire",
  };
  ...
  const admissionSteps = [
    { step: 1, title: "Remplir le formulaire en ligne", description: "Complétez le formulaire de candidature avec toutes les informations requises.", icon: ClipboardList },
    { step: 2, title: "Soumettre les documents", description: "Téléchargez les documents requis (acte de naissance, photos, etc.).", icon: FileText },
    { step: 3, title: "Examen du dossier", description: "Notre équipe examine votre candidature dans un délai de 5 à 10 jours ouvrables.", icon: Clock },
    { step: 4, title: "Notification de décision", description: "Vous recevrez une notification par email concernant l'acceptation de votre dossier.", icon: Mail },
    { step: 5, title: "Finalisation de l'inscription", description: "En cas d'acceptation, procédez au paiement des frais et finalisez l'inscription.", icon: CreditCard },
  ];
  ...
  return (
    <div className="min-h-screen bg-muted/30">
        ...
        <div className="container mx-auto px-4 py-12 relative">
          ...
          <div className="flex flex-col md:flex-row items-center gap-6">
            ...
            <div className="text-center md:text-left">
              ...
              <p className="text-primary-foreground/80 text-lg">Guide d'Inscription - Informations Pratiques</p>
            </div>
          </div>
        </div>
      </header>
      <div className="container mx-auto px-4 py-8 space-y-8">
        {currentYear && (
          <Card className="border-primary/20 bg-primary/5"><CardContent className="py-4"><div className="flex items-center justify-between flex-wrap gap-4"><div className="flex items-center gap-3"><Calendar className="w-5 h-5 text-primary" /><span className="font-medium">Année académique en cours:</span><Badge variant="default" className="text-base">{currentYear.name}</Badge></div><Link to={`/admissions/${tenantSlug}`}><Button size="lg">Postuler maintenant<ArrowRight className="w-4 h-4 ml-2" /></Button></Link></div></CardContent></Card>
        )}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <Card><CardHeader><CardTitle className="flex items-center gap-2"><ClipboardList className="w-5 h-5 text-primary" />Processus d'Admission</CardTitle><CardDescription>Suivez ces étapes pour finaliser votre inscription</CardDescription></CardHeader><CardContent><div className="space-y-6">{admissionSteps.map((step, index) => (<div key={step.step} className="flex gap-4"><div className="flex flex-col items-center"><div className="w-10 h-10 rounded-full bg-primary text-primary-foreground flex items-center justify-center font-bold">{step.step}</div>{index < admissionSteps.length - 1 && <div className="w-0.5 h-full bg-primary/20 mt-2" />}</div><div className="flex-1 pb-6"><div className="flex items-center gap-2 mb-1"><step.icon className="w-4 h-4 text-primary" /><h3 className="font-semibold">{step.title}</h3></div><p className="text-muted-foreground text-sm">{step.description}</p></div></div>))}</div></CardContent></Card>
            <Card><CardHeader><CardTitle className="flex items-center gap-2"><FileText className="w-5 h-5 text-primary" />Documents Requis</CardTitle><CardDescription>Préparez ces documents avant de soumettre votre candidature</CardDescription></CardHeader><CardContent><ul className="space-y-3">{requiredDocuments.map((doc, index) => (<li key={index} className="flex items-start gap-3"><CheckCircle2 className="w-5 h-5 text-green-500 flex-shrink-0 mt-0.5" /><span>{doc}</span></li>))}</ul></CardContent></Card>
            {levels && levels.length > 0 && (<Card><CardHeader><CardTitle className="flex items-center gap-2"><BookOpen className="w-5 h-5 text-primary" />Niveaux d'Études Disponibles</CardTitle></CardHeader><CardContent><div className="grid grid-cols-2 md:grid-cols-3 gap-3">{levels.map((level: any) => (<div key={level.id} className="p-3 rounded-lg bg-muted/50 border text-center"><GraduationCap className="w-6 h-6 text-primary mx-auto mb-2" /><p className="font-medium">{level.name}</p></div>))}</div></CardContent></Card>)}
      ...
      <footer className="bg-muted/50 border-t border-border py-6 mt-8"><div className="container mx-auto px-4 text-center"><p className="text-sm text-muted-foreground">© {new Date().getFullYear()} {tenant.name}. Tous droits réservés.</p></div></footer>
    </div>
  );
};
