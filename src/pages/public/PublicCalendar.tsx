      ...
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <h2 className="text-2xl font-display font-bold text-foreground">Événements à venir</h2>
            ...
            {eventsLoading ? (
              ...
              <Card>
                <CardContent className="p-12 text-center">
                  ...
                  <p className="text-muted-foreground">Les événements seront publiés prochainement.</p>
                </CardContent>
              </Card>
            )}
          </div>
          ...
          <div className="space-y-6">
            <h2 className="text-2xl font-display font-bold text-foreground">Périodes scolaires</h2>

            {terms && terms.length > 0 ? (
              <div className="space-y-4">
                {terms.map((term) => {
                  const isActive = isAfter(today, parseISO(term.start_date)) && isBefore(today, parseISO(term.end_date));

                  return (
                    <Card key={term.id} className={isActive ? "border-primary shadow-md" : ""}>
              ...
              <Card className="bg-muted/50">
                <CardContent className="p-4">
                  ...
                  <p className="text-sm text-muted-foreground mb-1">
                    <strong>Début:</strong> {format(parseISO(academicYear.start_date), "d MMMM yyyy", { locale: fr })}
                  </p>
                  <p className="text-sm text-muted-foreground">
                    <strong>Fin:</strong> {format(parseISO(academicYear.end_date), "d MMMM yyyy", { locale: fr })}
                  </p>
                </CardContent>
              </Card>
            )}
