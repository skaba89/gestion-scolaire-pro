...
function StatItem({ item, index }: { item: any; index: number }) {
  const color = item.color || '#60a5fa';
  const numericValue = typeof item.value === 'number'
    ? item.value
    : parseInt(String(item.value).replace(/\D/g, '')) || 0;
  const suffix = typeof item.value === 'string'
    ? String(item.value).replace(/[\d.,]/g, '')
    : item.suffix || '';
  const { count, ref } = useAnimatedCounter(numericValue);

  return (
    <div key={index} className="group" ref={ref}>
      {item.icon && (
        <div
...
/** Same real photos as GallerySection above ({url, caption} items,
 * per-section, uploaded independently of the tenant's landing gallery),
 * presented as an auto-advancing slideshow instead of a static grid —
 * requested alongside "des bannières avec photos qui défilent" (audit
 * 2026-08-28). Never fabricates images: renders nothing if the admin
 * hasn't added any real item yet, same as GallerySection. */
export function CarouselSection({ section }: { section: PublicPageSection }) {
  const items = section.items || [];
  const s = section.settings || {};
  ...
  const [emblaRef, emblaApi] = useEmblaCarousel(
    { loop: true },
    autoplay ? [Autoplay({ delay: 5000, stopOnInteraction: false, stopOnMouseEnter: true })] : [],
  );
  ...
  return (
    <section className="py-16 md:py-24 bg-white">
      <div className="container mx-auto px-4 sm:px-6">
        ...
        <div className="relative overflow-hidden rounded-2xl max-w-4xl mx-auto" ref={emblaRef}>
          <div className="flex">
            {items.map((item: any, i: number) => {
              const imageUrl = typeof item === 'string' ? item : item.url || item.image;
              const caption = typeof item === 'string' ? '' : item.caption || '';
              ...
              <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2">
                {items.map((_: any, i: number) => (
                  <button
                    ...
                    style={{
                      height: '0.5rem',
                      backgroundColor: i === selectedIndex ? '#ffffff' : 'rgba(255,255,255,0.5)',
