...
const Programs = () => {
  ...
  const formatAmount = (value: number): string => {
    ...
    if (currencyConfig.position === "before") return `${currencyConfig.symbol}${formattedNumber}`;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="container mx-auto px-4 py-16"><Card className="bg-gradient-hero text-primary-foreground border-0"><CardContent className="p-8 md:p-12 text-center"><Target className="w-12 h-12 mx-auto mb-4 opacity-80" /><h2 className="text-2xl md:text-3xl font-display font-bold mb-4">Prêt à commencer votre parcours ?</h2><p className="text-primary-foreground/80 mb-6 max-w-xl mx-auto">Rejoignez notre établissement et bénéficiez d'une formation de qualité adaptée à vos objectifs.</p><div className="flex flex-wrap justify-center gap-4"><Link to={`/admissions/${tenantSlug}`}><Button size="lg" className="bg-sky hover:bg-sky/90"><UserPlus className="w-5 h-5 mr-2" />Déposer ma candidature</Button></Link><Link to={`/info/${tenantSlug}`}><Button size="lg" variant="outline" className="border-primary-foreground/30 text-primary-foreground hover:bg-primary-foreground/10" style={{ color: "white" }}>En savoir plus</Button></Link></div></CardContent></Card></section>
