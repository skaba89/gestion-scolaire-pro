    ...
    const handleDeleteAccount = async () => {
        ...
        try {
            ...
            toast.success('Demande de suppression envoyée', {
                description: 'Un administrateur examinera votre demande sous 48h',
            });
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                                Fichier lisible et réutilisable
                            </p>
                        </div>
                        <Button
                                ...
                                {exportHistory.slice(0, 5).map((export_item, index) => (
                                    <div key={index} className="flex items-center justify-between text-sm">
                                        <span className="text-muted-foreground">
                                            {new Date(export_item.created_at).toLocaleDateString('fr-FR', {
                                                year: 'numeric',
                                                month: 'long',
                                                day: 'numeric',
                                                hour: '2-digit',
                                        ...
                                        <span className="text-xs text-muted-foreground">
                                            {export_item.new_values?.export_size_kb
                                                ? `${Math.round(export_item.new_values.export_size_kb)} KB`
                                                : '-'}
                                        </span>
                                    </div>
                                ))}
                ...
                <CardContent>
                    <div className="space-y-3">
                        <div className="flex justify-between items-center">
                            <span className="text-sm">Factures et paiements</span>
                            <span className="text-sm font-medium">10 ans</span>
                        </div>
                        <Separator />
                        <div className="flex justify-between items-center">
                            <span className="text-sm">Notes et présences</span>
                            <span className="text-sm font-medium">Permanent</span>
                        </div>
                        <Separator />
                        <div className="flex justify-between items-center">
                            <span className="text-sm">Messages et notifications</span>
                            <span className="text-sm font-medium">Supprimable</span>
                        </div>
                        <Separator />
                        <div className="flex justify-between items-center">
                            <span className="text-sm">Historique de connexion</span>
                            <span className="text-sm font-medium">5 ans</span>
                        </div>
                    </div>
                    <Alert className="mt-4">
                        <AlertDescription className="text-xs">
                            Base légale : Code de commerce Art. L123-22 (factures), Code de l'éducation Art. D211-10 (notes)
                        </AlertDescription>
                    </Alert>
                </CardContent>
            </Card>
            ...
            <Card className="border-destructive">
                ...
                <CardContent className="space-y-4">
                    ...
                    {deletionRequests.length > 0 && (
                        <Alert className={deletionRequests[0].status === 'PENDING' ? "bg-muted" : ""}>
                            ...
                            <AlertDescription className="flex items-center justify-between">
                                ...
                                <span className="text-xs text-muted-foreground">
                                    {new Date(deletionRequests[0].requested_at).toLocaleDateString()}
                                </span>
                            </AlertDescription>
                        </Alert>
                    )}
                    ...
                    <Button
                        ...
                        disabled={deletionRequests.some(r => r.status === 'PENDING')}
                    >
                        <Trash2 className="h-4 w-4" />
                        {deletionRequests.some(r => r.status === 'PENDING')
                            ? 'Demande en cours de traitement'
                            : 'Demande la suppression de mon compte'}
                    </Button>
                </CardContent>
            </Card>
            ...
            <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Cette action est irréversible. Vos données personnelles seront définitivement anonymisées.
                        </DialogDescription>
                    </DialogHeader>
