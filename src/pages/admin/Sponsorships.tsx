...
export default function Sponsorships() {
  ...
  const { data: students } = useQuery({
    ...
    queryFn: async (): Promise<Student[]> => {
      ...
      const query: any = supabase.from("students").select("id, first_name, last_name, registration_number");
      const { data, error } = await query.eq("tenant_id", tenant.id).eq("status", "ACTIVE");
      if (error) throw error;
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: typeof formData & { id?: string }) => {
      if (data.id) {
        ...
        const { error } = await supabase.from("student_sponsorships").insert({
          tenant_id: tenant?.id,
          sponsor_id: data.sponsor_id,
          sponsored_id: data.sponsored_id,
          academic_year_id: data.academic_year_id || null,
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("student_sponsorships").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["student-sponsorships"] });
