...
export const InventoryDashboard = () => {
    ...
    const stockByCategory = categories?.map(cat => ({
        name: cat.name,
        value: items?.filter(item => item.category_id === cat.id).reduce((acc, item) => acc + item.stock_quantity, 0) || 0
    })).filter(cat => cat.value > 0) || [];

    const valueByCategory = categories?.map(cat => ({
        name: cat.name,
        value: items?.filter(item => item.category_id === cat.id).reduce((acc, item) => acc + (item.stock_quantity * item.unit_price), 0) || 0
    })).filter(cat => cat.value > 0) || [];
    ...
    const movementData = last7Days.map(date => {
        const dayTrans = transactions?.filter(t => t.created_at.startsWith(date)) || [];
        return {
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">
                            {(items?.reduce((acc, item) => acc + (item.stock_quantity * item.unit_price), 0) || 0).toLocaleString()} €
                        </div>
                    </CardContent>
                </Card>
                <Card>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">
                            {transactions?.filter(t => t.transaction_type === 'IN' && new Date(t.created_at) > new Date(Date.now() - 7 * 86400000)).reduce((acc, t) => acc + t.quantity, 0) || 0}
                        </div>
                    </CardContent>
                </Card>
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">
                            {Math.abs(transactions?.filter(t => t.transaction_type === 'OUT' && new Date(t.created_at) > new Date(Date.now() - 7 * 86400000)).reduce((acc, t) => acc + t.quantity, 0) || 0)}
                        </div>
                    </CardContent>
                </Card>
            </div>
