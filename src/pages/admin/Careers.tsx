...
export default function Careers() {
  ...
  const offerMutation = useMutation({
    mutationFn: async (data: any) => {
      if (selectedOffer) {
        await apiClient.put(`/hr/job-offers/${selectedOffer.id}/`, data);
      } else {
        await apiClient.post("/hr/job-offers/", { ...data, tenant_id: tenant?.id });
      }
    },
    onSuccess: () => {
  ...
  const deleteOfferMutation = useMutation({
    mutationFn: async (id: string) => {
      await apiClient.delete(`/hr/job-offers/${id}/`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-job-offers"] });
  ...
  const eventMutation = useMutation({
    mutationFn: async (data: any) => {
      if (selectedEvent) {
        await apiClient.put(`/hr/career-events/${selectedEvent.id}/`, data);
      } else {
        await apiClient.post("/hr/career-events/", { ...data, tenant_id: tenant?.id });
      }
    },
    onSuccess: () => {
  ...
  const deleteEventMutation = useMutation({
    mutationFn: async (id: string) => {
      await apiClient.delete(`/hr/career-events/${id}/`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-career-events"] });
  ...
  const updateAppStatusMutation = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      await apiClient.put(`/hr/job-applications/${id}/`, { status });
    },
    onSuccess: () => {
