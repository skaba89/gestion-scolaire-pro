...
export default function ExecutiveDashboard() {
    ...
    const { data: riskScoresData, refetch: refetchRiskScores, isLoading: isLoadingRisk } = useQuery({
        ...
        queryFn: async () => {
            const { data: currentTerm } = await apiClient.get("/schedule/terms/", {
                params: { is_current: true },
            });

            const termData = (currentTerm as any[])?.[0];
            if (!termData) return { total: 0, critical: 0, high: 0, moderate: 0, low: 0 };

            const { data } = await apiClient.get("/analytics/risk-scores/", {
                params: { term_id: termData.id },
            });
    ...
    const { data: cashFlowForecasts, refetch: refetchForecasts } = useQuery({
        ...
        queryFn: async () => {
            const { data } = await apiClient.get("/analytics/cash-flow-forecasts/", {
                params: {
                    forecast_date_after: new Date().toISOString().split('T')[0],
    ...
    const handleExportPDF = () => {
        ...
        generateDashboardPDF({
            ...
            academic: {
                successRate: `${(academicData?.overallSuccessRate ?? 0).toFixed(1)}%`,
                totalStudents: academicData?.totalStudents || 0,
                averageGrade: `${(academicData?.averageGrade ?? 0).toFixed(2)}/20`,
                failingStudents: academicData?.failingStudents || 0,
            },
            operational: {
                attendanceRate: `${(operationalData?.studentAttendanceRate ?? 0).toFixed(1)}%`,
                teacherAttendance: `${(operationalData?.teacherAttendanceRate ?? 0).toFixed(1)}%`,
                enrollments: operationalData?.totalEnrollments || 0,
                dropoutRate: `${(operationalData?.dropoutRate ?? 0).toFixed(1)}%`,
