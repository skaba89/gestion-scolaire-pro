...
const AlumniMentors = () => {
  ...
  const saveMentorMutation = useMutation({
    mutationFn: async (mentorData: any) => {
      ...
      if (editingMentor?.id) {
        await apiClient.put(`/hr/alumni-mentors/${editingMentor.id}/`, payload);
      } else {
        await apiClient.post("/hr/alumni-mentors/", payload);
      }
    },
    onSuccess: () => {
  ...
  const deleteMentorMutation = useMutation({
    mutationFn: async (mentorId: string) => {
      await apiClient.delete(`/hr/alumni-mentors/${mentorId}/`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-alumni-mentors", tenant?.id] });
  ...
  const updateRequestMutation = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: MentorshipStatus }) => {
      ...
      await apiClient
        .put(`/hr/mentorship-requests/${id}/`, updateData);
    },
    onSuccess: () => {
