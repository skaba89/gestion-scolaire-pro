...
const Certificates = () => {
  ...
  const fetchStudentsByClassroom = async () => {
    ...
    const { data: enrollments } = await supabase
      .from("enrollments")
      .select(`
        student:students(id, first_name, last_name, registration_number, date_of_birth)
      `)
      .eq("tenant_id", tenant!.id)
  ...
  const fetchStudentEnrollment = async (studentId: string) => {
    ...
    if (data) {
      setEnrollment({
        ...data,
        academic_year: data.academic_year as { id: string; name: string },
        classroom: data.classroom as { id: string; name: string },
        level: data.level as { id: string; name: string } | null
      });
    }
  };
