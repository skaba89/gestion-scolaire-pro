...
export default function LiveAttendancePage() {
  ...
  const fetchData = async () => {
    ...
    if (selectedClassroom !== "all") {
      enrollmentQuery = enrollmentQuery.eq("class_id", selectedClassroom);
    }
      ...
      const channel = supabase
        ...
        .on(
          ...
          (payload) => {
            ...
            queryClient.setQueryData<CheckIn[]>(
              ["admin-student-check-ins", tenant?.id, dayStart, dayEnd],
              (prev) => {
                if (!prev) return [newCheckIn];
                // Avoid duplicates if optimistic update already added it
                if (prev.some(c => c.id === newCheckIn.id)) return prev;
                return [newCheckIn, ...prev.filter(c => !c.id.toString().startsWith('temp-'))];
              }
            );
          }
        )
