...
export const TeacherRiskDashboard = () => {
    ...
    const { data: teacherClasses } = useQuery({
        ...
        queryFn: async () => {
            ...
            return data?.map(cs => ({
                id: cs.class_id,
                name: (cs.classes as { name: string }).name
            })) || [];
        },
    ...
    const { data: riskScores, isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            return (data || []).map((score) => ({
                ...
                student_name: `${score.students.first_name} ${score.students.last_name}`,
                class_name: score.classes.name,
    ...
    const { data: recommendations } = useQuery({
        ...
        queryFn: async () => {
            ...
            const recs = await recommendationEngine.generateTeacherRecommendations(
                classIds[0], // Use first class for now
                riskScores.map(rs => ({
        ...
        enabled: !!riskScores && riskScores.length > 0
    });
