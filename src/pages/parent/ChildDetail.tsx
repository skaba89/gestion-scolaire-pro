  ...
  const { data: relation } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await apiClient.get('/parents/children/', {
        params: { parent_id: user.id, student_id: studentId },
      });
    ...
    try {
      ...
      const transcriptData = {
        tenant: {
          ...
          address: tenant?.address || "Adresse de l'établissement"
        },
        student: {
      ...
      const { data } = await apiClient.post('/parents/transcript/generate/', transcriptData);
        ...
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row md:items-center gap-6">
            <div className="w-20 h-20 rounded-full bg-primary/10 flex items-center justify-center border-2 border-primary/20">
              {student?.photo_url ? (
                <img src={student.photo_url} alt={`${student.first_name} ${student.last_name}`} className="w-full h-full rounded-full object-cover" />
              ) : (
                <span className="text-2xl font-bold text-primary">{student?.first_name?.[0]}{student?.last_name?.[0]}</span>
              )}
            </div>
            <div className="flex-1">
              <h1 className="text-2xl font-display font-bold">{student?.first_name} {student?.last_name}</h1>
              <div className="flex flex-wrap items-center gap-2 mt-2">
                {student?.registration_number && <Badge variant="outline">{student.registration_number}</Badge>}
                {enrollment && <Badge variant="secondary">{(enrollment.classrooms as any)?.name || (enrollment.levels as any)?.name}</Badge>}
                {enrollment && <Badge variant="outline" className="bg-muted">{(enrollment.academic_years as any)?.name}</Badge>}
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 pt-6 border-t">
            {student?.date_of_birth && <div className="flex items-center gap-2 text-sm"><Calendar className="w-4 h-4 text-muted-foreground" /><span>{format(new Date(student.date_of_birth), "dd MMM yyyy", { locale: fr })}</span></div>}
            {student?.email && <div className="flex items-center gap-2 text-sm"><Mail className="w-4 h-4 text-muted-foreground" /><span className="truncate">{student.email}</span></div>}
            {student?.phone && <div className="flex items-center gap-2 text-sm"><Phone className="w-4 h-4 text-muted-foreground" /><span>{student.phone}</span></div>}
            {student?.address && <div className="flex items-center gap-2 text-sm"><MapPin className="w-4 h-4 text-muted-foreground" /><span className="truncate">{student.address}</span></div>}
          </div>
        </CardContent>
      </Card>
            ...
            <CardContent className="h-[250px] pt-4">
              {grades && grades.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={grades.slice(0, 15).reverse().map((g: any) => ({ date: format(new Date(g.created_at), "dd/MM"), score: (g.score / (g.assessments?.max_score || 20)) * 20, subject: g.assessments?.subjects?.name }))}>
                  ...
                  {grades?.slice(0, 20).map((grade: any) => (
                    <div key={grade.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground">{grade.assessments?.subjects?.name} • {grade.assessments?.type}</p>
                        {grade.assessments?.date && <p className="text-xs text-muted-foreground">{format(new Date(grade.assessments.date), "dd MMM yyyy", { locale: fr })}</p>}
                      </div>
                      <div className="text-right"><span className={`text-lg font-bold ${getAverageColor((grade.score / (grade.assessments?.max_score || 20)) * 20)}`}>{grade.score}/{grade.assessments?.max_score || 20}</span></div>
                    </div>
                  ))}
                </div>
              )}
                    ...
                    {attendance?.slice(0, 30).map((record: any) => (
                      <div key={record.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                        <div className="flex items-center gap-3">
                          <div className={`w-3 h-3 rounded-full ${record.status === "PRESENT" ? "bg-success" : record.status === "ABSENT" ? "bg-destructive" : record.status === "LATE" ? "bg-warning" : "bg-info"}`} />
