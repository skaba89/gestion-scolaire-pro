...
const Invoices = () => {
  ...
  // 2. Fetch Invoices via parentsService
  ...
  const handleOnlinePayment = async (invoice: any, method: 'stripe' | 'paytech' = 'stripe') => {
    ...
    try {
      ...
      const functionName = method === 'stripe' ? "create-invoice-payment" : "create-paytech-payment";
    ...
    <div className="space-y-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <Select value={selectedChild} onValueChange={setSelectedChild}>
                  <SelectTrigger className="w-full sm:w-72 bg-card border-primary/20 font-medium">
                  ...
                  {invoices.map((invoice) => {
                    ...
                    return (
                      <TableRow key={invoice.id} className="hover:bg-muted/30 transition-colors">
                        <TableCell>
                          <div className="flex flex-col gap-1">
                            ...
                            {invoice.has_payment_plan && (
                              <Badge variant="secondary" className="text-[9px] w-fit bg-primary/10 text-primary border-primary/20">
                                <CalendarDays className="w-3 h-3 mr-1" />
                                {invoice.installments_count} échéances
                              </Badge>
                            )}
                          </div>
                        </TableCell>
                        ...
                        <TableCell className={`font-bold text-sm ${remaining > 0 ? "text-orange-600" : "text-green-600"}`}>
                        ...
                        <TableCell>
                          <Badge className={`${status?.color || ""} border-none shadow-none text-[10px] uppercase font-bold`}>
                          ...
                          <div className="flex items-center justify-end gap-2">
                            {(onlinePaymentsEnabled || mobileMoneyEnabled) && canPay && (
                              <Button
                                ...
                                className="h-8 gap-1.5 shadow-sm bg-primary hover:bg-primary/90"
                              >
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto custom-scrollbar shadow-2xl border-primary/10 p-0 overflow-hidden rounded-2xl">
          <DialogHeader className="p-6 bg-muted/30 border-b">
            <DialogTitle className="flex items-center gap-3 font-display text-xl">
              ...
              Facture {detailInvoice?.invoice_number}
            </DialogTitle>
          </DialogHeader>
          ...
          {detailInvoice && (
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-muted/40 rounded-xl border border-primary/5">
                  <p className="text-[10px] uppercase font-bold text-muted-foreground mb-1">{StudentLabel}</p>
                  <p className="font-bold text-sm">
                    {(detailInvoice.students as any)?.first_name} {(detailInvoice.students as any)?.last_name}
                  </p>
                </div>
                <div className="p-4 bg-muted/40 rounded-xl border border-primary/5">
                  <p className="text-[10px] uppercase font-bold text-muted-foreground mb-1">Statut</p>
                  <Badge className={`${statusConfig[detailInvoice.status as PaymentStatus]?.color || ""} border-none shadow-none text-[10px] font-bold`}>
                    {statusConfig[detailInvoice.status as PaymentStatus]?.label || detailInvoice.status}
                  </Badge>
                </div>
              </div>
              ...
              <div className="bg-muted/20 border-2 border-primary/5 rounded-2xl p-6 space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-bold">{formatCurrency(Number(detailInvoice.total_amount))}</span>
                </div>
                <div className="flex justify-between text-sm text-green-600">
                  <span className="font-medium">Montant Payé</span>
                  <span className="font-bold">{formatCurrency(Number(detailInvoice.paid_amount || 0))}</span>
                </div>
                <div className="flex justify-between text-lg font-bold pt-3 border-t-2 border-dashed border-primary/10">
                  {(() => {
                    const detailRemaining = Number(detailInvoice.total_amount) - Number(detailInvoice.paid_amount || 0);
                    return (
                      <>
                        <span className={detailRemaining > 0 ? "text-orange-600 font-display" : "text-green-600 font-display"}>Reste à payer</span>
                        <span className={detailRemaining > 0 ? "text-orange-600 font-display" : "text-green-600 font-display"}>
                          {formatCurrency(detailRemaining)}
                        </span>
                      </>
                    );
                  })()}
              ...
              {Array.isArray(detailInvoice.items) && detailInvoice.items.length > 0 && (
                <div className="space-y-2">
                  ...
                  <div className="border border-primary/5 rounded-xl overflow-hidden shadow-sm">
                    <Table>
                      <TableHeader className="bg-muted/50">
                        <TableRow>
                          <TableHead className="text-xs h-9">Description</TableHead>
                          <TableHead className="text-center text-xs h-9">Qté</TableHead>
                          <TableHead className="text-right text-xs h-9">Total</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {detailInvoice.items.map((item: any, idx: number) => {
                          const isPenalty = item.isPenalty || item.name?.toLowerCase().includes("pénalité");
                          return (
                            <TableRow key={idx} className={cn("hover:bg-transparent", isPenalty && "bg-red-50 text-red-900")}>
                              <TableCell className="py-3 text-sm font-medium flex items-center gap-2">
                                {isPenalty && <AlertCircle className="w-3.5 h-3.5 text-red-600" />}
                                {item.name}
                              </TableCell>
                              <TableCell className="py-3 text-center text-sm">{item.quantity}</TableCell>
                              <TableCell className="py-3 text-right text-sm font-bold">{formatCurrency(Number(item.total))}</TableCell>
                            </TableRow>
                          );
                        })}
                      </TableBody>
                ...
                <div className="space-y-2">
                  ...
                  <div className="border border-primary/5 rounded-xl overflow-hidden shadow-sm">
                    <Table>
                      <TableHeader className="bg-muted/50">
                        <TableRow>
                          <TableHead className="text-xs h-9">N°</TableHead>
                          <TableHead className="text-right text-xs h-9">Montant</TableHead>
                          <TableHead className="text-xs h-9">Date limite</TableHead>
                          <TableHead className="text-xs h-9">Statut</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {schedules.map((schedule) => (
                          <TableRow key={schedule.id}>
                            <TableCell className="py-3">
                              <span className="font-mono text-xs font-bold bg-muted p-1 rounded">#{schedule.installment_number}</span>
                            </TableCell>
                            ...
                            <TableCell className="py-3">
                              <Badge
                                className={`text-[9px] font-bold uppercase shadow-none border-none ${schedule.status === "PAID" ? "bg-green-100 text-green-700" :
                                  schedule.status === "OVERDUE" || new Date(schedule.due_date) < new Date() ? "bg-red-100 text-red-700" :
                                    "bg-orange-100 text-orange-700"
                                  }`}
                              >
                                {schedule.status === "PAID" ? "Payé" :
                                  schedule.status === "OVERDUE" || new Date(schedule.due_date) < new Date() ? "En retard" :
                                    "À venir"}
                              </Badge>
                            </TableCell>
                          </TableRow>
              ...
              <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4">
                <Button
                  variant="outline"
                  className="font-bold text-sm h-11 px-6 rounded-xl shadow-sm"
                  onClick={() => handleDownloadPDF(detailInvoice)}
                  disabled={downloadingId === detailInvoice.id}
                >
                  {downloadingId === detailInvoice.id ? (
                ...
                {onlinePaymentsEnabled && (Number(detailInvoice.total_amount) - Number(detailInvoice.paid_amount || 0)) > 0 && (
                  <Button
                    className="font-bold text-sm h-11 px-6 rounded-xl shadow-md bg-primary hover:bg-primary/90"
                    onClick={() => handleOnlinePayment(detailInvoice)}
                    disabled={payingInvoiceId === detailInvoice.id}
                  >
                    {payingInvoiceId === detailInvoice.id ? (
                      <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                    ) : (
      ...
      <Dialog open={paymentMethodDialog.open} onOpenChange={(open) => setPaymentMethodDialog({ ...paymentMethodDialog, open })}>
        <DialogContent className="sm:max-w-[400px]">
          <DialogHeader>
            <DialogTitle className="text-center font-display">Choisir le mode de paiement</DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-1 gap-4 py-4">
            {onlinePaymentsEnabled && (
              <Button
                variant="outline"
                className="h-20 flex flex-col items-center justify-center gap-2 border-2 hover:border-primary hover:bg-primary/5 group transition-all"
                onClick={() => handleOnlinePayment(paymentMethodDialog.invoice, 'stripe')}
              >
                <div className="flex items-center gap-2">
                  <CreditCard className="w-6 h-6 text-primary" />
                  <span className="font-bold">Carte Bancaire</span>
                </div>
                <span className="text-[10px] text-muted-foreground uppercase tracking-widest">Visa, Mastercard</span>
              </Button>
            )}
            {mobileMoneyEnabled && (
              <Button
                variant="outline"
                className="h-20 flex flex-col items-center justify-center gap-2 border-2 hover:border-primary hover:bg-primary/5 group transition-all"
                onClick={() => handleOnlinePayment(paymentMethodDialog.invoice, 'paytech')}
              >
                <div className="flex items-center gap-2">
                  <Smartphone className="w-6 h-6 text-primary" />
                  <span className="font-bold">Mobile Money</span>
                </div>
                <span className="text-[10px] text-muted-foreground uppercase tracking-widest">Wave, Orange, MTN</span>
              </Button>
            )}
          </div>
        </DialogContent>
