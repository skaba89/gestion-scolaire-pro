...
const ReportCards = () => {
  ...
  const filteredGrades = grades?.filter((grade: any) => {
    ...
    const term = terms?.find((trm: any) => trm.id === selectedTerm);
  ...
  const handleDownloadReportCard = async () => {
    if (selectedChild === "all" || selectedTerm === "all") {
      toast.error(t("parentReportCards.selectChildAndPeriod"));
      return;
    }

    if (!enrollment) {
      toast.error(t("parentReportCards.noActiveEnrollment"));
      return;
    }
    ...
    try {
      ...
      if (data?.html) {
        ...
        if (printWindow) {
          ...
          toast.error(t("parentReportCards.popupBlocked"));
        }
      } else {
        throw new Error(t("parentReportCards.invalidFormat"));
      }

      toast.success(t("parentReportCards.bulletinGenerated"));
    } catch (error: any) {
      toast.error(error.message || t("parentReportCards.bulletinError"));
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div><h1 className="text-2xl font-display font-bold text-foreground">{t("parentReportCards.pageTitle")}</h1><p className="text-muted-foreground">{t("parentReportCards.pageSubtitle")}</p></div>
      </div>
      ...
      {selectedChild && selectedChild !== "all" && children?.length > 0 && (() => {
        ...
        if (!isMinor) return (<Alert variant="destructive"><AlertCircle className="h-4 w-4" /><AlertDescription>{t("parentReportCards.minorOnly")}</AlertDescription></Alert>);
        return null;
      })()}
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex flex-col sm:flex-row items-center gap-4">
              <div className="w-full sm:w-auto flex-1 flex items-center gap-2"><Users className="w-4 h-4 text-muted-foreground shrink-0" /><Select value={selectedChild} onValueChange={setSelectedChild}><SelectTrigger className="w-full"><SelectValue placeholder={t("parentReportCards.selectChild")} /></SelectTrigger><SelectContent><SelectItem value="all">{t("parentReportCards.allChildren")}</SelectItem>{children?.map((relation: any) => { const student = relation.students as any; return (<SelectItem key={relation.student_id} value={relation.student_id}>{student?.first_name} {student?.last_name}</SelectItem>); })}</SelectContent></Select></div>
              <div className="w-full sm:w-auto flex-1 flex items-center gap-2"><Calendar className="w-4 h-4 text-muted-foreground shrink-0" /><Select value={selectedTerm} onValueChange={setSelectedTerm}><SelectTrigger className="w-full"><SelectValue placeholder={t("parentReportCards.period")} /></SelectTrigger><SelectContent><SelectItem value="all">{t("parentReportCards.allPeriods")}</SelectItem>{terms?.map((term: any) => (<SelectItem key={term.id} value={term.id}>{term.name}</SelectItem>))}</SelectContent></Select></div>
              <Button onClick={handleDownloadReportCard} disabled={generatingPdf || selectedChild === "all" || selectedTerm === "all"} className="w-full sm:w-auto">{generatingPdf ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Download className="w-4 h-4 mr-2" />}{t("parentReportCards.generatePdf")}</Button>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      {isLoading ? (
        ...
        <Card><CardContent className="py-12 text-center"><FileText className="w-12 h-12 text-muted-foreground/50 mx-auto mb-4" /><p className="text-muted-foreground">{t("parentReportCards.noGradesAvailable")}</p></CardContent></Card>
      ) : (
        <div className="space-y-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <Card><CardContent className="p-4"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center"><BarChart3 className="w-5 h-5 text-primary" /></div><div><p className="text-2xl font-bold">{filteredGrades?.length || 0}</p><p className="text-xs text-muted-foreground">{t("parentReportCards.gradesDisplayed")}</p></div></div></CardContent></Card>
          </div>
          <Card>
            <CardHeader><CardTitle className="flex items-center gap-2"><BarChart3 className="w-5 h-5" />{t("parentReportCards.gradesDisplayed")} - {terms?.find((term: any) => term.id === selectedTerm)?.name || t("parentReportCards.allPeriods")}</CardTitle></CardHeader>
            <CardContent>
              <div className="space-y-4">
                {filteredGrades?.map((grade: any) => {
                  ...
                  const termName = terms?.find((trm: any) => gradeDate && new Date(trm.start_date) <= gradeDate && new Date(trm.end_date) >= gradeDate)?.name;
                  return (
                    <div key={grade.id} className="flex items-center justify-between p-4 rounded-lg bg-muted/50 transition-all hover:bg-muted/80">
                      ...
                      <div className="text-right"><p className="font-bold text-sm">{t("parentReportCards.on")} {assessment?.max_score}</p><p className={`text-xs font-medium ${percentage >= 50 ? 'text-green-600' : 'text-red-600'}`}>{percentage.toFixed(0)}%</p></div>
                    </div>
                  );
                })}
              </div>
