...
const ParentPreRegistration = () => {
  ...
  const { data: children, isLoading: childrenLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data?.map(d => d.students).filter(Boolean) || []) as StudentWithEnrollments[];
    },
    enabled: !!user?.id && !!tenant?.id,
  });

  // Récupérer l'année académique suivante
  ...
  // Vérifier les pré-inscriptions existantes pour chaque enfant
    ...
    queryFn: async () => {
      ...
      for (const child of children) {
        const { data } = await supabase
          ...
          .eq("student_first_name", child.first_name)
          .eq("student_last_name", child.last_name)
          .maybeSingle();

        if (data) {
          applications[child.id] = data;
        }
      }
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Inscrivez vos {studentsLabel} pour l'année suivante</p>
        </div>

        <Card>
          <CardContent className="p-8 text-center">
            <AlertCircle className="w-12 h-12 text-muted-foreground mx-auto mb-4" />
            <h2 className="text-lg font-semibold mb-2">Aucun {studentLabel} associé</h2>
            <p className="text-muted-foreground">
              Votre compte n'est pas encore associé à des {studentsLabel}.
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <h2 className="text-lg font-semibold mb-2">Inscriptions non ouvertes</h2>
            <p className="text-muted-foreground">
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-display font-bold text-foreground">Pré-Réinscription</h1>
        <p className="text-muted-foreground">
          Inscrivez vos {studentsLabel} pour l'année {nextAcademicYear.name}
        </p>
      </div>
      ...
      <Card className="bg-blue-500/5 border-blue-500/20">
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-medium text-foreground">Pré-réinscription en ligne</p>
              <p className="text-sm text-muted-foreground">
                Soumettez la demande de pré-réinscription pour réserver la place de vos {studentsLabel}.
                L'administration validera ensuite votre demande.
              </p>
            </div>
          </div>
        </CardContent>
            ...
            <Card key={child.id}>
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  {existingApp && (
                    <Badge className={statusConfig[existingApp.status as keyof typeof statusConfig]?.color || "bg-muted"}>
                      {statusConfig[existingApp.status as keyof typeof statusConfig]?.label || existingApp.status}
                    </Badge>
                  )}
                </div>
                <CardDescription>
                  {child.registration_number && `N° Étudiant: ${child.registration_number}`}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    ...
                    <p className="font-medium">{currentEnrollment?.levels?.name || "Non inscrit"}</p>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Niveau suggéré:</span>
                    <p className="font-medium">{nextLevel?.name || "À déterminer"}</p>
                  </div>
                </div>
                ...
                {existingApp ? (
                  <div className="bg-muted/50 rounded-lg p-3">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span>
                        Demande soumise le{" "}
                        {new Date(existingApp.submitted_at || existingApp.created_at).toLocaleDateString('fr-FR')}
                      </span>
                    </div>
                  </div>
                ) : (
                  ...
                  >
                    ...
                    Pré-inscrire pour {nextAcademicYear.name}
                  </Button>
                )}
              </CardContent>
            </Card>
      ...
      <Dialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Pré-Réinscription</DialogTitle>
            <DialogDescription>
              Confirmez la pré-réinscription de {selectedChild?.first_name} {selectedChild?.last_name} pour l'année {nextAcademicYear?.name}.
            </DialogDescription>
          </DialogHeader>
                ...
                <SelectContent>
                  {levels?.map((level) => (
                    <SelectItem key={level.id} value={level.id}>
                      {level.name}
                      {selectedChild && level.id === getNextLevel(selectedChild)?.id && " (suggéré)"}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
