    ...
    return useMutation({
        ...
        onSuccess: (data) => {
            ...
            const message = data.emailSent
                ? "Compte créé et identifiants envoyés par email"
                : `Compte créé avec succès. Mot de passe généré : ${data.generatedPassword || "vérifiez vos emails"}`;
...
export const useConvertToAccount = (tenantId: string) => {
    ...
    return useMutation({
        ...
        onSuccess: (data) => {
            ...
            if (!data.emailSent) {
                toast.warning(`Compte créé. Échec de l'email. Mot de passe : ${data.generatedPassword}`, {
                    duration: 10000
                });
            } else {
                toast.success("Compte créé et email envoyé.");
            }
        },
        onError: (error: any) => {
