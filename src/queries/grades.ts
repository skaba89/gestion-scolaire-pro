    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            const response = await apiClient.get<Grade[]>("/grades/", {
                // NOTE: The backend may ignore the assessment_id filter parameter.
                // Client-side filtering may be needed as a fallback until the
                // backend endpoint supports filtering by assessment_id.
