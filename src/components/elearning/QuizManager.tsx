...
export function QuizManager({ quizId, quizTitle }: QuizManagerProps) {
  ...
  const { data: questions = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get<any[]>("/quiz-questions/", {
        params: { quiz_id: quizId, ordering: "order_index" },
      });
  ...
  const createQuestionMutation = useMutation({
    mutationFn: async (data: typeof questionForm) => {
      await apiClient.post("/quiz-questions/", {
        tenant_id: tenant!.id,
        quiz_id: quizId,
        ...
        {questions.length === 0 && (
          <Card className="border-dashed">
            <CardContent className="py-8 text-center">
              <HelpCircle className="h-10 h-10 mx-auto mb-3 text-muted-foreground" />
