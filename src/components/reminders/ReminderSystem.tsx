...
export const ReminderSystem = () => {
  ...
  const { data: invoiceData } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Get pending invoices - use correct uppercase status values
  ...
  const { data: eventData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: events } = await supabase
        .from("school_events")
        .select("id, title, description, start_date, end_date, event_type, location")
        .eq("tenant_id", tenant.id)
    ...
    invoiceData.forEach((inv: any) => {
      ...
      if (daysUntilDue <= preferences.daysBeforePayment || isOverdue) {
        reminders.push({
          ...
          description: `${inv.students?.first_name} ${inv.students?.last_name} - ${inv.amount}€`,
          dueDate,
