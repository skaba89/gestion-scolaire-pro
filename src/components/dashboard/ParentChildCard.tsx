...
export const ParentChildCard = ({ student, tenantId }: ParentChildCardProps) => {
  ...
  const { data: enrollment } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get<any[]>("/enrollments/", {
        params: {
  ...
  const { data: attendanceStats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await apiClient.get<any[]>("/attendance/", {
        params: {
          student_id: student.id,
          date_gte: thirtyDaysAgo.toISOString().split("T")[0],
        },
      });

      if (!data) return { present: 0, absent: 0, late: 0, total: 0, rate: 0 };
  ...
  const { data: homeworkStats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: homework } = await apiClient.get<any[]>("/homework/", {
        params: {
      ...
      if (!homework) return { pending: 0, submitted: 0, graded: 0 };
      ...
      // Get submissions
      const { data: submissions } = await apiClient.get<any[]>("/homework-submissions/", {
        params: {
          student_id: student.id,
          homework_id__in: homeworkIds.join(","),
        },
      });
