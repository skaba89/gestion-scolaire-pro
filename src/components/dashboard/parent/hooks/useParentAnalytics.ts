...
export const useParentAnalytics = (studentId: string, tenantId: string) => {
    ...
    const { data: homeworkDataRaw = [] } = useQuery({
        ...
        queryFn: async () => {
          ...
          submitted_at,
          grade,
          homework:homework(title, max_points, due_date)
        `)
                .eq("student_id", studentId)
    ...
    const { data: achievements = [] } = useQuery({
        ...
        queryFn: async () => {
            const { data, error } = await supabase
                ...
                .select(`
          earned_at,
          achievement:achievement_definitions(name, description, icon, rarity, points_reward)
        `)
                .eq("student_id", studentId)
    ...
    const { data: pointsData = [] } = useQuery({
        ...
        queryFn: async () => {
            try {
                const { data, error } = await supabase
                    .from("student_achievements")
                    .select("achievement:achievement_definitions(points_reward)")
                    .eq("student_id", studentId);
                if (error) return [];
                return data?.map(d => ({
