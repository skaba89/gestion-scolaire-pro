...
export const StudentImport = () => {
  ...
  const importMutation = useMutation({
    mutationFn: async (students: Partial<any>[]) => {
      ...
      const { data: tenantData } = await apiClient.get(`/tenants/${tenant?.id}/`);

      const settings = tenantData?.data?.settings || tenantData?.settings as Record<string, unknown> | null;
      ...
      for (let i = 0; i < students.length; i++) {
        ...
        const { error } = await apiClient.post('/students/', insertData);

        if (error) {
          results.errors.push(`Ligne ${i + 2}: ${error.response?.data?.detail || error.message}`);
        } else {
          results.success++;
        }
      }
      ...
      if (results.success > 0) {
        ...
        await apiClient.patch(`/tenants/${tenant?.id}/`, { settings: updatedSettings });
      }
