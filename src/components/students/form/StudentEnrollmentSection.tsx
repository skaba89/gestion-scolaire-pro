...
export const StudentEnrollmentSection = ({
    ...
    useEffect(() => {
        const fetchClasses = async () => {
            ...
            name, 
            level_id,
            classroom_departments!inner(department_id)
          `)
                    .eq("level_id", levelId)
                    .eq("classroom_departments.department_id", departmentId);
                ...
                const { data } = await supabase.from("classrooms").select("*").eq("level_id", levelId);
                setClasses(data || []);
            } else {
                setClasses([]);
            }
    ...
    useEffect(() => {
        const fetchOptionalSubjects = async () => {
            if (classId && classId !== "none") {
                ...
                // Ideally, we'd filter by what's available for this class/level
                // For now, let's just show all optional subjects or as per establishment settings
                if (data) {
