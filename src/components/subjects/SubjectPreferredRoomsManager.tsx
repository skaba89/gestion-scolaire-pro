...
export const SubjectPreferredRoomsManager = ({ subjectId, tenantId }: SubjectPreferredRoomsManagerProps) => {
    ...
    const availableRooms = useMemo(() => {
        const assignedIds = new Set(subjectRooms.map((sr) => sr.room_id));
        return allRooms.filter((r) => !assignedIds.has(r.id));
    ...
    const handleAddRoom = async () => {
        ...
        setAdding(true);
        try {
            const { error } = await supabase.from("subject_preferred_rooms").insert({
                tenant_id: tenantId,
            ...
            toast({
                title: "Erreur",
                description: error.message || "Impossible d'ajouter la salle",
                variant: "destructive",
            });
        } finally {
            setAdding(false);
    ...
    const handleRemoveRoom = async (linkId: string) => {
        try {
            const { error } = await supabase.from("subject_preferred_rooms").delete().eq("id", linkId);
            if (error) throw error;
            ...
            toast({
                title: "Erreur",
                description: error.message || "Impossible de retirer la salle",
                variant: "destructive",
            });
        }
    };
