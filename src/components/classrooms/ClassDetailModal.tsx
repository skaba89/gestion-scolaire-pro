...
export const ClassDetailModal = ({ isOpen, onClose, classroom }: ClassDetailModalProps) => {
    ...
    const fetchStudents = async () => {
        ...
        if (data) {
            setStudents(data.map((d: any) => d.students || d).filter(Boolean));
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-4xl p-0 overflow-hidden sm:rounded-2xl border-none shadow-2xl">
                <VisuallyHidden>
                    <DialogTitle>Détails de la classe - {classroom.name}</DialogTitle>
                    <DialogDescription>Informations détaillées, élèves et programme de la classe</DialogDescription>
                </VisuallyHidden>
