...
export function MessageReactions({ messageId, compact = false }: MessageReactionsProps) {
  ...
  const { data: reactions } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<any[]>(`/communication/messages/${messageId}/reactions/`);
  ...
  const addReactionMutation = useMutation({
    mutationFn: async (emoji: string) => {
      try {
        await apiClient.post(`/communication/messages/${messageId}/reactions/`, {
          emoji,
        });
      } catch (error: any) {
        if (error.response?.status === 409) {
          // Already exists, remove it
          await apiClient.delete(`/communication/messages/${messageId}/reactions/`, {
            params: { emoji }
          });
        } else {
          throw error;
