...
export function MessageSearchBar({ onSelectMessage, onClose }: MessageSearchBarProps) {
  ...
  const { data: results = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const response = await apiClient.get<any[]>("/communication/messages/search/", {
          params: { q: debouncedTerm, limit: 20 }
        });
        ...
        return (response.data || []).map((m: any) => ({
          ...
          sender: m.sender || { id: m.sender_id, first_name: '', last_name: '' },
          conversation_subject: m.conversation_subject,
        })) as SearchResult[];
      } catch {
        return [];
      }
