...
// ChatBot removed (rendered globally in App.tsx)
...
export const AdminLayout = () => {
  ...
  const { signOut, profile, hasRole, roles } = useAuth();
  ...
  // Stable refs for functions that shouldn't trigger re-renders
  const hasRoleRef = useRef(hasRole);
  hasRoleRef.current = hasRole;
  const getTenantUrlRef = useRef(getTenantUrl);
  getTenantUrlRef.current = getTenantUrl;

  // Redirection Logic for Onboarding — using stable deps only
  ...
  const allNavSections: NavSection[] = useMemo(() => [
    ...
    },
    {
      ...
      items: [
        ...
        { href: getTenantUrl("/admin/semesters"), label: "Semestres", icon: CalendarDays, permission: "terms:manage" },
        ...
        { href: getTenantUrl("/admin/faculties"), label: "Facultés", icon: Landmark, permission: "faculties:manage" },
  ...
  // Mobile bottom nav items - most used actions (memoized to prevent infinite re-renders)
  ...
  }), [t, getTenantUrl, StudentsLabel, roles]);
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      ...
      {/* AI Support Chat Widget is rendered globally in App.tsx */}
