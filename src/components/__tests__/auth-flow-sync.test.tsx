...
const mockSignInWithKeycloak = vi.fn();
...
describe("Auth page tenant-aware redirects", () => {
  beforeEach(() => {
    ...
    mockToast.mockReset();
    mockSignInWithKeycloak.mockReset();
  ...
  it("préserve le chemin demandé avant de lancer la connexion SSO", async () => {
    mockUseAuth.mockReturnValue({
      signInWithKeycloak: mockSignInWithKeycloak.mockResolvedValue({ error: null }),
      user: null,
      roles: [],
      isLoading: false,
    });
    ...
    await waitFor(() => {
      ...
      expect(mockSignInWithKeycloak).toHaveBeenCalled();
    });
  });
});
