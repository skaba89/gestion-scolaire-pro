...
export default function BatchBadgePrint({ onBadgesCreated }: BatchBadgePrintProps) {
  ...
  const fetchAcademicYear = async () => {
    ...
    const { data } = await apiClient.get<{ id: string; name: string }[]>("/academic-years/", {
      params: { tenant_id: tenant.id, is_current: true },
    });
    const ay = Array.isArray(data) ? data[0] : data;
    if (ay) {
      setAcademicYear(ay);
    }
  };
  ...
  const fetchStudentsInClassroom = async () => {
    ...
    const { data: academicYearDataRaw } = await apiClient.get<{ id: string; name: string }[]>("/academic-years/", {
      params: { tenant_id: tenant.id, is_current: true },
    });
    const academicYearData = Array.isArray(academicYearDataRaw) ? academicYearDataRaw[0] : academicYearDataRaw;
    ...
    const { data: enrollments } = await apiClient.get<any[]>("/enrollments/", {
      params: {
    ...
    const studentList = (enrollments || [])
      .map((e: any) => ({
        ...(e.student || e),
        classroomName,
        levelName: e.student?.level?.name || e.level?.name || levelName,
      }))
      .filter((s: any) => s && s.id) as StudentWithBadge[];

    // Get existing badges for these students
    if (studentList.length > 0) {
      const studentIds = studentList.map((s) => s.id);
      const { data: badges } = await apiClient.get<any[]>("/student-badges/", {
        params: {
  ...
  const handleGenerateMissingBadges = async () => {
    ...
    try {
      ...
      await apiClient.post("/student-badges/", newBadges);
  ...
  const handleDeleteBadge = async (badgeId: string, studentName: string) => {
    ...
    setDeleting(badgeId);
    try {
      await apiClient.delete(`/student-badges/${badgeId}/`);
