...
export const pedagogicalEngine = {
    ...
    async calculateClassResults(classId: string, termId: string, tenantId: string) {
        ...
        const coeffOverrides = new Map(overrides?.map(o => [o.subject_id, o.coefficient]) || []);

        // 3. Group by student
        const studentGrades = new Map<string, Grade[]>();
        gradesData.forEach((g: any) => {
            const list = studentGrades.get(g.student_id) || [];
            const subjectId = g.assessments.subject_id;
            const baseCoeff = g.assessments.subjects?.coefficient || 1;
            const coeff = coeffOverrides.get(subjectId) || baseCoeff;

            list.push({
                score: Number(g.score),
                max_score: Number(g.assessments.max_score),
                weight: Number(g.assessments.weight),
        ...
        for (const [studentId, grades] of studentGrades.entries()) {
            ...
            for (const [subjectId, sGrades] of bySubject.entries()) {
                ...
                // Save individual subject result (optional/async)
                // this.saveTermResult(...)
            }
    ...
    async saveTermResults(results: any[], tenantId: string, classId: string, termId: string) {
        ...
        const { error } = await supabase
            .from("term_results")
            .upsert(inserts, { onConflict: "student_id, term_id, subject_id" });
