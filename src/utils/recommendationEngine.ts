...
export const recommendationEngine = {
    ...
    async generateParentRecommendations(
        ...
        const studentName = student ? `${student.first_name}` : "votre enfant";
    ...
    async generateDirectionRecommendations(
        ...
        for (const classItem of classes) {
            ...
            if (enrollments) {
                ...
                if (riskScores) {
                    criticalCount += riskScores.filter(r => r.risk_level === "CRITICAL").length;
                    highCount += riskScores.filter(r => r.risk_level === "HIGH").length;
                }
            }
        }
